...
class Hand():
  ...
  def winner(self):
    ...
    winner=self.cards[0]
    for card in self.cards:
        # the winning card is not a trump
        if not winner.trump:
            if card.trump :
                winner=card
            ...
            if card.value>winner.value and card.trump:
                winner=card
