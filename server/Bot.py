...
if __name__=="__main__" :
  ...
  bob.count(card[const.COLORS[3]])
  assert(bob.counter[const.COLORS[3]]==1)

  bill = AdvancedBot()
  bill.counter
  bill.count(card[const.COLORS[2]])
  bill.count(card[const.COLORS[1]])
  assert(bill.counter[const.COLORS[2]]==1)
  assert(bill.counter[const.COLORS[1]]==1)
  bill.reinitialize()
  assert(bill.previouscounter[const.COLORS[2]]==1)
  assert(bill.previouscounter[const.COLORS[1]]==1)
  bill.count(card[const.COLORS[0]])
  assert(bill.counter[const.COLORS[0]]==1)
  assert(bill.previouscounter[const.COLORS[0]]==0)
  """
  print("Test OK")
